...
4. 最后输出 <UpdateVariable>…</UpdateVariable>：须符合下方「## 四、变量补丁约束」。**本局开局提示里已给出示例 Patch，若 user 楼层已写入同名路径，你方须写入完整 value 覆盖**（\`replace\` / \`add\` / \`insert\` 均可；本界面合并器中后二者对对象路径与 \`replace\` 等价）。按实际剧情微调数值与描写。
...
- **角色档案 value（类型硬约束，违反则变量无法写入）**：顶层键须齐全：**姓名、状态、角色简介、代表性发言、描写、爱好、当前内心想法、当前位置、性格、性癖、敏感点开发、隐藏性癖、身体信息、服装状态、身体部位物理状态、数值、身份标签、当前综合生理描述、参与活动记录**（**兼容**旧存档仅含 **敏感部位** 键时，界面会合并读取；新写入请用 **敏感点开发**；缺 **角色简介 / 代表性发言 / 爱好** 的旧档由 Schema 补默认）。
