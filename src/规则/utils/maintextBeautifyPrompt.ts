...
2. **局部美化**：对确有必要的**词或短短语**外包 <span class="th-…">…</span>；同一自然段内带 class 的 span **不宜超过 2～5 处**（含下条流光对白）。
3. **对白流光**：整句对白若以中文弯引号「……」或 ASCII 双引号 \`"……"\` 呈现，可将**引号与句内文字一并**外包 \`<span class="th-dialog-shimmer">「……」</span>\`（引号形式与原文一致）；**流光由宿主 CSS 实现**，叙事 <style> 中**不要**再写 \`.th-dialog-shimmer\` 或同名 @keyframes。同段流光对白 **不宜超过 2～3 处**。
